...
bool OpRunner::RunOp()
{
    ...
    string reduction;
    auto ret = aclnnInstanceNormGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], 
                                                 opDesc_->dataFormat, opDesc_->epsilon, 
    ...
    {
        Timer timer;
        if (aclnnInstanceNorm(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
            (void)aclrtDestroyStream(stream);
        ...
        if (aclrtSynchronizeStreamWithTimeout(stream, 10000) != SUCCESS) {
